...
data <- read_excel("C:\\Users\\Abit\\Downloads\\NSC-2025.xlsx", sheet="Sheet2")
...
```{r}
colnames(data) <- c("Kabupaten/Kota", "Y", paste0("X", 1:9)) #ganti nama peubah dengan x 1-9
head(data)
```
...
```{r}
plot(data$X4, data$Y,
     main = "Rata-rata lama sekolah vs Sanitasi layak",
     xlab = "RLS",
     ylab = "IPM")
```
...
Koef_det <- r^2
Koef_det #menunjukan keragaman yang bisa dijelaskan dari X4
```
...
Didapatkan nilai $\hat\beta_0 = 60.25436$ artinya nilai dugaan rataan indeks pembangunan manusia ketika sanitasi layak bernilai 0 persen adalah 60.254%, akan tetapi nilai 0 tidak ada dalam selang pengamatan sanitasi layak sehingga $\hat\beta_0$ tidak dapat didefinisikan lebih lanjut

Nilai $\hat\beta_0 = 0.1828016$ artinya nilai dugaan perubahan dugaan rataan Indeks Pembangunan Manusia jika sanitasi layak berubah satu tahun adalah sebesar 2.875%

Koefisien determinasi yang dihasilkan adalah  0.239335 atau 23.93% artinya keragaman persentase penduduk miskin yang dapat dijelaskan oleh persentase akses sanitasi layak adalah sebesar 88.84%, sisanya yaitu 11.16% dijelaskan oleh peubah lain yang tidak dimasukkan dalam model.
